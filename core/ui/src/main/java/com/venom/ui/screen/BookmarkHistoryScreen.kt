...
fun BookmarkHistoryScreen(
    ...
    onOcrItemClick: (OcrEntity) -> Unit = {
        navController.navigate("ocr") {
            popUpTo("ocr") { saveState = true }
    ...
    val viewResources = if (selectedTab == 0) ViewResources(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        when (contentType) {
            ContentType.TRANSLATION -> {
                val items by translationViewModel.items.collectAsState()
                TransBookmarkList(
                    items = items,
                    searchQuery = searchQuery,
            ...
            ContentType.OCR -> {
                val items by ocrViewModel.items.collectAsState()
                OcrBookmarkList(
                    items = items,
                    searchQuery = searchQuery,
...
enum class ContentType { TRANSLATION, OCR, PHRASEBOOK, DIALOG, STACKCARD }
enum class ViewType { BOOKMARKS, HISTORY }
